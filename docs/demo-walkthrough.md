...
> 日期：2026-08-13（初版） ｜ 2026-08-15 增补说明 ｜ 2026-08-17 增补说明
> 复现命令：`python3 phase2/demo.py`（零 GPU，Mac 本地即可）
> 演示完整链路：字段定义 → 画像引擎 → 检索引擎 → 建议卡。

## 增补说明（2026-08-17）

相比 8/15，当前系统又叠加了产品/体验优化：

- **核心建议段**：建议卡顶部「核心建议」先给首选策略一句话 + 关键风险提示（冲突/机制兜底/near-miss/未匹配字段），转述时先讲结论。
- **匹配分数可解释**：每条候选策略显示「匹配分数 + 构成」（语义命中率 + 版式命中率 + 状态加权），说明排序依据。
- **未匹配字段 → 最接近概念候选**：字段名无法匹配时，给出 top-3 模糊匹配概念候选（如 `bank` → bank.issuing/bank.beneficiary），引导人工确认语义而非干列字段名。
- **near-miss 相近经验**：标签命中但被 lane/单据类型等适用性条件拒绝的经验，以「相近但未命中」段落呈现并说明被拒原因。
- **输入容错**：advise.py 对非法 JSON/空 fields 输出友好报错（退出码 2）而非崩溃。

## 增补说明（2026-08-15）

本文档以 8/13 跑通的输出为快照，**CLAIM ID / status / 机制归属可能随后续数据更新而变**。要看最新输出，直接跑：

```bash
cd phase2 && python3 demo.py
```

相比初版（8/13），当前系统已叠加：

- **机制层两级检索**（Mechanism）：候选策略段落会带「归属机制」标注（如 MECH-0001 行级归组分段守恒），命中机制但当前字段类型无验证实例时，会出现「兜底推荐 + 谨慎」段落。
- **版式视觉确认**（可选）：profiler 接 `image_path` + `embedding_server` + `index_path` 时，画像摘要会增加「版式视觉确认：<单据类型>（<中文名>，<标注范围>标注）」+ 冲突警告。
- **字段语义向量兜底**（bge-m3）：profiler 别名表未命中时走向量兜底，命中标 `matched_by="vector"`。
- **单据中文名**：装箱单 / 海运单 / 托收 / 贷记通知等自动展示中文名。
- **lane 推断**：根据样例图所在目录的标注范围后缀（`_goods` / `_non_goods` / `_mixed`）自动推断 lane。

skill 形式的入口（自然语言触发）见 `skill-dev/training-preflight/` 或 `dist/training-preflight.zip`。
