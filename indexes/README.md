...
| 文件 | 大小 | 内容 | 构建脚本 |
|---|---|---|---|
| `layout_index.json` | ~98MB | 版式向量：8 单据 2145 图（dim 2048，qwen3-vl-embedding） | `layout_library/build_index.py` |
| `concept_index.json` | ~9.8MB | 字段语义概念向量：85 概念 431 片段（dim 1024，bge-m3） | `phase2/build_concept_index.py` |

## 为什么不进 git

这两个是**派生数据**（从源单据图片 + embedding 模型算出来的），体积大（合计 ~108MB），且可随时用构建脚本 + embedding 服务重建。进 git 会让仓库膨胀，对公开阅读者无意义。
...
- **重建**：
  ```bash
  # 版式索引（需 qwen3-vl-embedding 服务，公网 9030）
